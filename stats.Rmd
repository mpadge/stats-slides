...
```
```{r autotest1, eval = FALSE, echo = FALSE, collapse = TRUE}
library (autotest)
path <- file.path (here::here (), "demo")
...
x <- c ("[1] Parameter [x] is not specified as integer, yet only used as such;\n",
        ...
        "[2] Parameter [x] of function [addtwo] is only used a a single\n",
        "    numeric value, but responds to vectors of length > 1\n\n",
        "[3] Parameter [y] is not specified as integer, yet only used as such;\n",
        "    please use '1L' for integer, or 1.0 for non-integer values.\n\n",
        "[4] Parameter [y] of function [addtwo] is only used a a single\n",
        "    numeric value, but responds to vectors of length > 1")
message (x)
```
...
- `pkgcheck` and associated `html` reports facilitate rapid review of standards compliance
